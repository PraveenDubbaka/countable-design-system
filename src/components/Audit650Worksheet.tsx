// Form 650 — Subsequent Events (CAS 560 / AU-C 560)
...
const dWrittenReps = (): ProcRow[] => [
  makeProcRow("Obtain written representations that all events occurring subsequent to the period end and for which the AFRF requires adjustment or disclosure have been adjusted or disclosed (Form 580)."),
];
const dDisclosures = (): ProcRow[] => [
  makeProcRow("Review the final F/S and evaluate whether subsequent events are appropriately reflected in accordance with the AFRF (see Form 680 / FRF 913)."),
    ...
    >
      ...
      >
        {data.events.length === 0 ? (
          <p className="px-6 py-4 text-xs text-muted-foreground">No subsequent events recorded. Each identified event flows into Form 580 (representations), Form 535 (TCWG) and Form 700 (auditor's report).</p>
        ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {data.revisedReportIssued === "Y" && (
            <div className="md:col-span-3"><Label>Revised report — dating & references</Label>
              <Textarea disabled={locked} value={data.revisedReportNotes} onChange={e => setData(d => ({ ...d, revisedReportNotes: e.target.value }))} className="text-sm min-h-[64px]" placeholder="Record the new report date (≥ date of approval of amended F/S) and link to Form 700." />
            </div>
          )}
        </div>
      </WorksheetSection>
