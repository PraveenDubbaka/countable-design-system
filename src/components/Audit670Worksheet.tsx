// Form 670 — Use of Journal Entries (CAS 240 / AU-C 240)
...
const dPreparation = (): ProcRow[] => [
  makeProcRow("Review understanding of journal entries (Form 535) and Form 520 for any RMM relating to the entity's financial-reporting process, management override or other fraud risks."),
];
...
const dExistence = (): ProcRow[] => [
  ...
  makeProcRow("7. Identify unusual / inappropriate JEs during the period — based on results above and RMM (Form 520), consider extending testing over the full reporting period.", "E (C AV)"),
];
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-6">
          ...
          {fraudRisks.length > 0 && (
            <div className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded px-2.5 py-1">
              {fraudRisks.length} fraud risk{fraudRisks.length === 1 ? "" : "s"} identified in Form 520 — consider extending JE testing.
            </div>
          )}
        </div>
      </WorksheetSection>
      ...
      <LinkedRisksCard overallRisk={overall} risks={jeRisks}
        emptyHint="No JE / management-override risks tagged in Form 520. Management-override is presumed; document attributes selected."
      ...
      >
        {data.entries.length === 0 ? (
          <p className="px-6 py-4 text-xs text-muted-foreground">No journal entries selected yet. Each entry tested records the preparer, approver, attributes triggered, rationale and outcome — feeding the Form 240 fraud assessment.</p>
        ) : (
        ...
        {followUps.length > 0 && (
          <div className="px-6 py-3 border-t border-border bg-amber-50 text-xs text-amber-800">
            {followUps.length} entr{followUps.length === 1 ? "y" : "ies"} flagged for follow-up — ensure each is resolved before concluding, and consider impact on Form 240 / Form 450.
          </div>
        )}
      </WorksheetSection>
      ...
      <WorksheetSection title="Evaluation & escalation">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div><Label>Reported to TCWG (Form 535)?</Label><YNSelect value={data.reportToTcwg} onChange={v => setData(d => ({ ...d, reportToTcwg: v as YN }))} locked={locked} /></div>
          ...
          {data.managementOverrideIndicated === "Y" && (
            <div className="md:col-span-3"><Label>Management override — nature & response</Label>
              <Textarea disabled={locked} value={data.managementOverrideNotes} onChange={e => setData(d => ({ ...d, managementOverrideNotes: e.target.value }))} className="text-sm min-h-[72px]" placeholder="Update Form 240 fraud assessment and Form 520 RMM accordingly." />
            </div>
          )}
          {data.reportToTcwg === "Y" && (
